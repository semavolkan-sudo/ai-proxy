export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
          ...
          html: `
            <div style="font-family:sans-serif;max-width:480px;margin:0 auto;background:#070711;color:#fff;padding:32px;border-radius:16px;">
              ...
              <div style="text-align:center;margin:32px 0;">
                <a href="${resetLink}" style="background:linear-gradient(135deg,#d4a853,#f0c060);color:#08080f;text-decoration:none;padding:14px 32px;border-radius:12px;font-weight:700;font-size:16px;display:inline-block;">
                  Şifremi Sıfırla
                </a>
              </div>
              <p style="color:#888899;font-size:12px;line-height:1.6;">Bu bağlantı 1 saat geçerlidir. Eğer bu talebi siz yapmadıysanız bu e-postayı görmezden gelebilirsiniz.</p>
    ...
    if (action === 'verify-reset-token') {
      ...
      if (new Date(userData.reset_token_expiry) < new Date()) {
        return res.status(200).json({ ok: false, reason: 'token_expired' });
      }
